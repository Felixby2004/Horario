...
export default function VistaLaboratorioPage() {
  ...
  const cargarDatos = async () => {
    try {
      setCargando(true);
      const [resLaboratorios, resConfig] = await Promise.all([
        fetch('/api/ambientes?tipo=laboratorio'),
        fetch('/api/configuracion')
      ]);

      const [dataLaboratorios, dataConfig] = await Promise.all([
        resLaboratorios.json(),
        resConfig.json()
      ]);

      if (dataLaboratorios.exito) setLaboratorios(dataLaboratorios.datos);
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">💻 Horarios por Laboratorio</h1>
        ...
        <div className="bg-white rounded-lg shadow p-4 mb-6">
          ...
          <select
            value={laboratorioSeleccionado?.id_ambiente || ''}
            onChange={(e) => {
          ...
          >
            ...
            {laboratorios.map((l: any) => (
              <option key={l.id_ambiente} value={l.id_ambiente}>
                {l.codigo} - {l.nombre} (Capacidad: {l.capacidad})
              </option>
            ))}
          </select>
        </div>
  ...
  if (horarios.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">No hay horarios para este laboratorio</p>
      </div>
    );
  }
      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="bg-gray-200">
            ...
            {DIAS.map(dia => (
              <th key={dia} className="border p-2 text-center font-bold bg-purple-100">{dia}</th>
            ))}
          </tr>
        </thead>
        <tbody>
                    ...
                    {items.length > 0 ? (
                      <div className="space-y-1">
                        {items.map((item, idx) => (
                          <div
                            key={idx}
                            className="p-1 bg-purple-100 text-purple-900 rounded text-xs font-semibold"
                          >
